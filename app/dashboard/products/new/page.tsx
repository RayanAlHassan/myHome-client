...
export default function AddProductPage() {
  ...
  // ✅ images vs glb are separate and clearly named
  const [images, setImages] = useState<FileList | null>(null);   // field: images
  const [modelGlb, setModelGlb] = useState<File | null>(null);   // field: modelGlb
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!loggedIn || !user) return setError("You must be logged in to add a product.");
    if (!["admin", "vendore"].includes(user.role)) return setError("You do not have permission to add a product.");
    if (!BASE_URL) return setError("Missing NEXT_PUBLIC_API_URL");
    ...
    if (!images || images.length === 0) {
      return setError("Please upload at least one product image.");
    }

    // ✅ Optional: validate glb extension if user picked one
    if (modelGlb && !modelGlb.name.toLowerCase().endsWith(".glb")) {
      return setError("3D Model must be a .glb file.");
    }
    ...
    // ✅ must be "images" (same as multer field name)
    Array.from(images).forEach((img) => formData.append("images", img));

    // ✅ must be "modelGlb" (same as multer field name)
    if (modelGlb) formData.append("modelGlb", modelGlb);
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="border border-gray-300 dark:border-gray-600 rounded p-3">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">
            Upload 1 to 5 images. This field sends as <b>images</b>.
          </p>
        ...
        <div className="border border-gray-300 dark:border-gray-600 rounded p-3">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">
            Upload one GLB file. This field sends as <b>modelGlb</b>.
          </p>

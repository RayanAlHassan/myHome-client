...
import Link from "next/link";
import { Menu, LogOut, Moon, Sun, Home, Globe } from "lucide-react"; // ✅ add Globe
import { useState, useEffect } from "react";
import { useAuth } from "@/hooks/useAuth";
import { useTheme } from "./theme-provider";
import { Button } from "./ui/button";
...
export default function Sidebar({ onCollapseChange }: SidebarProps) {
  ...
  // ✅ same design, just add roles
  const menuItems = [
    { label: "Dashboard", icon: <Home className="w-5 h-5" />, href: "/dashboard", roles: ["admin"] },
    { label: "Users", icon: "👤", href: "/dashboard/users", roles: ["admin"] },
    { label: "Categories", icon: "📁", href: "/dashboard/categories", roles: ["admin"] },
    { label: "Subcategories", icon: "📝", href: "/dashboard/subcategories", roles: ["admin"] },
    { label: "Products", icon: "📅", href: "/dashboard/products", roles: ["admin", "vendore", "customer", "specialNeedCustomer"] },
  ];

  // ✅ filter by role (admin sees all, others see only Products)
  ...
  return (
    ...
    >
      ...
      {/* ✅ Make sidebar scrollable on small/short screens */}
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="py-4">
          ...
          {/* ✅ NEW button: Go to Website (keeps same design style as menu links) */}
          <div className="mt-2 px-2">
            <Link
              href="/"
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 group hover:bg-muted text-muted-foreground hover:text-foreground ${
                isCollapsed ? "justify-center" : ""
              }`}
              title={isCollapsed ? "Website" : ""}
            >
              <span className="text-lg flex items-center justify-center">
                <Globe className="w-5 h-5" />
              </span>
              {!isCollapsed && <span className="font-medium truncate">Website</span>}
            </Link>
          </div>
        </div>
      </div>

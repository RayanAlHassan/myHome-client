...
import { products, subCategories, categories } from "@/lib/data"; // ✅ import subCategories and categories
...
export function ARDemo() {
  ...
  // Get products that have AR models
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="font-serif text-3xl md:text-4xl font-bold mb-4">
              Try AR with Our Products
            </h2>
            <p className="text-lg text-muted-foreground leading-relaxed max-w-2xl mx-auto">
              Select a product below to see it in augmented reality. On mobile
              devices, you can place products in your actual space!
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {arEnabledProducts.map((product) => {
              ...
              const cat = subCat
                ? categories.find((c) => c._id === subCat.categoryId)
                : null;

              return (
                <div
                  key={product._id} // was product.id
                  className="bg-card rounded-xl border border-border overflow-hidden hover:shadow-lg transition-shadow"
                >
                  <div className="aspect-square relative bg-muted">
                    <Image
                      src={product.image || "/placeholder.svg"}
                      alt={product.title} // was product.name
                      fill
                  ...
                  <div className="p-4 space-y-3">
                    <div>
                      <h3 className="font-semibold text-lg mb-1">
                        {product.title} {/* was product.name */}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        {cat?.title || "Uncategorized"} {/* was product.category */}
                      </p>
                    </div>
